...
const MeetingRoom = () => {
    ...
    <section className='relative h-screen w-full overflow-hidden pt-4 text-white'>
      ...
      <div className="fixed bottom-0 flex w-full items-center justify-center gap-5">
        ...
        <DropdownMenu>
          ...
          <DropdownMenuTrigger className='cursor-pointer rounded-2xl bg-[#19232d] px-4 py-2 hover:bg-[#4c535b]'> <LayoutList size={20} className='text-white'>
            </LayoutList></DropdownMenuTrigger>
          </div>
  ...
  <DropdownMenuContent className='border-dark-1 bg-dark-1 text-white '>
    {['Grid ' , 'Speaker-left', 'Speaker-Right'].map((item,index)=>(
     <div key={index} >
      <DropdownMenuItem className='cursor-pointer'  onClick={()=>{setlayout(item.toLowerCase() as CallLayoutTyoe)}}>
        {item}
      </DropdownMenuItem>
...
<CallStatsButton></CallStatsButton>
 <button onClick={()=>{
  ...
  <div className='cursor-pointer rounded-2xl bg-color-[#19232d] px-4 py-2 hover:bg-[$4c535b]'>
